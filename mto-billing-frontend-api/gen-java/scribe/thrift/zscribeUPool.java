...
public class zscribeUPool extends scribeUPool {

  private static final Object lock = new Object();
  private static Map<String, zscribeUPool> instances = new HashMap<String, zscribeUPool>();
  ...
  public static zscribeUPool getInstance(String serviceName, String masterHostPort, String slaveHostPort) {
    return getInstance(serviceName, masterHostPort, slaveHostPort, 0, 0, 0, 0);
  }
  public static zscribeUPool getInstance(String serviceName, String masterHostPort, String slaveHostPort,int maxCons, int maxConsPerHost, int initConns, int timeout) {
    String key = mapCode(serviceName, masterHostPort, slaveHostPort);
    if (!instances.containsKey(key)) {
      synchronized(lock) {
        if (!instances.containsKey(key)) {
          instances.put(key, new zscribeUPool(serviceName, masterHostPort, slaveHostPort, maxCons, maxConsPerHost, initConns, timeout));
        }
      }
    }
  ...
  public zscribeUPool() {}
  public zscribeUPool(String serviceName, String masterHostPort, String slaveHostPort, int maxCons, int maxConsPerHost, int initConns, int timeout) {
    if (maxCons > 0) {
    ...
    zscribe.Client.Factory factory = new zscribe.Client.Factory();
  ...
  public synchronized  void changeConfig(String serviceName, String oldMasterHostPort,String oldSlaveHostPort, String masterHostPort, String slaveHostPort) {
    instances.put(mapCode(serviceName, masterHostPort, slaveHostPort),new zscribeUPool(serviceName, masterHostPort, slaveHostPort,maxCons, maxConsPerHost, initConns, timeout));
    instances.remove(mapCode(serviceName, oldMasterHostPort, oldSlaveHostPort));
  }

  public void Log2(final List<LogEntry> messages) throws TException{
      ...
      public OperationResult<Void> execute() throws ConnectionException {
        return context.getConnectionPool().executeWithFailover(new AbstractOperationImpl() {
          ...
          protected Void internalExecute(TServiceClient client) throws Exception {
            ((zscribe.Iface) client).Log2(messages);
            return null;
          }
