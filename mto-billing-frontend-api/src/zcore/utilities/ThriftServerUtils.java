...
//    public final static int DEFAULT_NUM_SELECTOR_THREADS = 16;
...
//        DEFAULT_NUM_WORKER_THREADS = Math.max(32, Runtime.getRuntime().availableProcessors());
    }
     ...
     * @param port port number on which the Thrift server will listen
     * @param clientTimeoutMillisecs
     * @param maxFrameSize max size (in bytes) of a transport frame, supply
     * {@code <=0} value to let the method choose a default {@code maxFrameSize}
     * value (which is 1Mb)
     * @param maxWorkerThreads max number of worker threads, supply {@code <=0}
     * value to let the method choose a default {@code maxWorkerThreads} value
     * (which is {@code Math.max(2, Runtime.getRuntime().availableProcessors())}
     * )
     * @return
     ...
     * {@code <=0} value to let the method choose a default {@code maxFrameSize}
     * value (which is 1Mb)
     * @param maxReadBufferSize max size (in bytes) of read buffer, supply
     * {@code <=0} value to let the method choose a default
     * {@code maxReadBufferSize} value (which is 16Mb)
     * @return
     * @throws TTransportException
     */
     ...
     * {@code <=0} value to let the method choose a default
     * {@code maxReadBufferSize} value (which is 16Mb)
     * @param numWorkerThreads number of worker threads, supply {@code <=0}
     * value to let the method choose a default {@code numWorkerThreads} value
     * (which is {@code Math.max(2, Runtime.getRuntime().availableProcessors())}
     * )
     * @return
     * @throws TTransportException
     ...
     * {@code <=0} value to let the method choose a default
     * {@code maxReadBufferSize} value (which is 16Mb)
     * @param numSelectorThreads number of selector threads, supply {@code <=0}
     * value to let the method choose a default {@code numSelectorThreads} value
     * (which is {@code 2} )
     * @param numWorkerThreads number of worker threads, supply {@code <=0}
     * value to let the method choose a default {@code numWorkerThreads} value
     * (which is {@code Math.max(2, Runtime.getRuntime().availableProcessors())}
